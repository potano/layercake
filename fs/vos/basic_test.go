...
func TestParsePath(T *testing.T) {
	for _, test := range []struct{path string; want []string} {
		{"", []string{}},
		{"/", []string{"/"}},
		{"a", []string{"a"}},
		{"a/b", []string{"a", "b"}},
...
func checkNSDevices(ns *namespaceType, T *testing.T, testDevices nsTestDevices) {
	...
	for _, spec := range testDevices {
		...
		for i, inode := range fs_inodes {
			...
			default:
				T.Fatalf("device %s, inode %d: unknown entry type %d", spec.majmin, i,
					want.tp)
			}
...
func TestResolveRootPath(T *testing.T) {
	...
	mount, dirInode, inode, name, err := mos.resolvePath(mos.cwd, "/", true)
	if err != nil {
